...
class UFlow(object):
  ...
  def __init__(
    ...
    Args:
      ...
      checkpoint_dir1: str, location to checkpoint_first_best_model
      checkpoint_dir2: str, location to checkpoint_second_best_model
    ...
    self.update_checkpoint_dir1(checkpoint_dir1)
    self.update_checkpoint_dir2(checkpoint_dir2)
  ...
  def update_checkpoint_dir1(self, checkpoint_dir1):
    self._manager_1 = tf.train.CheckpointManager(
        self._checkpoint, directory=checkpoint_dir1, max_to_keep=1)

  def update_checkpoint_dir2(self, checkpoint_dir2):
    self._manager_2 = tf.train.CheckpointManager(
        self._checkpoint, directory=checkpoint_dir2, max_to_keep=1)
  ...
  def save_1(self):
    """Saves a model checkpoint."""
    #self.update_checkpoint_dir1(checkpoint_dir1)
    self._manager_1.save()

  def save_2(self):
    """Saves a model checkpoint."""
    #self.update_checkpoint_dir2(checkpoint_dir2)
    self._manager_2.save()
  ...
  def _loss_and_grad(self,
    ...
    with tf.GradientTape() as tape:
      ...
      #flow = self.batch_infer_no_tf_function(batch)

      # g_loss = dis.g_loss(batch, self)
      #
      # losses['g-loss'] = g_loss
      # losses['total-loss'] = losses['total-loss'] + 1.0 * g_loss
  ...
  def compute_loss(self,
    ...
    # flow_1 = self.batch_infer_no_tf_function(batch)
    #
    # g_loss = dis.g_loss(batch, flow_1)
    ...
    # print('yes not_occluded_masks:', not_occluded_masks)
    # print('yes valid_warp_masks:',valid_warp_masks)

    # print('yes', images[0])
    # print('yes', images[1])
    # print('yes', warped_images[(0, 1, 'augmented-student')])

    #print('yes: ', not_occluded_masks)
    mask11 = tf.stop_gradient(not_occluded_masks[(0, 1, 'augmented-student')][0] * valid_warp_masks[(0, 1, 'augmented-student')][0])

    flow1 = flows[(0, 1, 'augmented-student')][0]
    flow2 = flows[(1, 0, 'augmented-student')][0]

    g_loss1 = dis.g_loss(images[0], images[1], flow1 + flow2, mask11)

    mask12 = tf.stop_gradient(not_occluded_masks[(1, 0, 'augmented-student')][0] * valid_warp_masks[(1, 0, 'augmented-student')][0])

    g_loss2 = dis.g_loss(images[1], images[0], flow2 + flow1, mask12)
    ...
    losses['g-loss'] = 0.001 * g_loss
    losses['total-loss'] = losses['total-loss'] + losses['g-loss']
