    ...
    if FLAGS.train_on:
        ...
        while True:
            ...
            for num, batch in zip(range(num_steps), train_it):
                ...
                flow = uflow.batch_infer_no_tf_function(images)

                # start_time_train_step = time.time()

                # d_loss, out_put_true, out_put_false, g_loss = dis.train_step(images, flow)

                d_loss, out_put_true, out_put_fake, g_loss = dis.test(images, flow)

                #print('num: ', num, 'd_loss: ', d_loss.numpy(), 'out_put_true: ', out_put_true.numpy(), 'out_put_fake: ', out_put_fake.numpy(), 'g_loss: ', g_loss.numpy())

                num_true = num_true + out_put_true[0][0].numpy()

                num_fake = num_fake + out_put_fake[0][0].numpy()

                print('num: ', num, 'out_put_true: ', out_put_true.numpy(), 'out_put_fake: ', out_put_fake.numpy(),
                      'num_true:', num_true / num, 'num_fake:', num_fake / num)
